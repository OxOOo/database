// modified from https://github.com/TsinghuaDatabaseGroup/GTree/blob/master/src/gtree_new_p2p/GPTree.cpp
#include<iostream>
#include<cstdio>
#include<cstdlib>
#include<cstring>
...
const char Edge_File[]="driver_dataset/BJ.edge";//第一行两个整数n,m表示点数和边数，接下来m行每行三个整数U,V,C表示U->V有一条长度为C的边
const char Node_File[]="driver_dataset/BJ.node";//共N行每行一个整数两个实数id,x,y表示id结点的经纬度(但输入不考虑id，只顺序从0读到n-1，整数N在Edge文件里)
